...
class ObsManager {
 public:
  ...
  void SetObsBaseData(
      const ObsData& obs_data);  // 设置基础数据，RINEX版本号，起止时间
  void SetObsType(const ObsType& obs_type);      // 设置观测类型
  void AddObsData(const EpochData& epoch_data);  // 添加历元数据
  void ProcessObsData(const std::function<void(const ObsData&)>& processor)
      const;  // 处理观测数据
